...
func TestLoginUserControllerValid(t *testing.T) {
	// Setup
	e := echo.New()
	reqBody, err := json.Marshal(map[string]string{"email": "adh@gmail.com", "password": "123"})
	assert.NoError(t, err)
...
func TestLoginUserControllerInvalidPassword(t *testing.T)  {
	// Setup
	e := echo.New()
	reqBody, err := json.Marshal(map[string]string{"email": "adh@gmail.com", "password": "9856"})
	assert.NoError(t, err)
	req := httptest.NewRequest(http.MethodPost, "/users/login", bytes.NewReader(reqBody))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
